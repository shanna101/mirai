...
if (!text)
    return api.sendMessage("Add text lmao", event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://leyscoders-api.herokuapp.com/api/ttp2?text=${text}&apikey=MIMINGANZ`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}