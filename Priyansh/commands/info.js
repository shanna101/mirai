...
module.exports.run = async function ({ api, event, args, Users, permssion, getText ,Threads}) {
  ...
  const listNDH = NDH || config.NDH ||  [];
  {
    ...
    var callback = () => 
      api.sendMessage({ body: `»»»»» BOT INFORMATION «««««

Bot name: 
Prefix: [ ${PREFIX} ]
Command Loaded: [ ${commands.size} ]
Thread of BOT: [ ${global.data.allThreadID.length} ]
Bot user: [ ${global.data.allUserID.length} ]
Bot owner: [ ${global.config.BOTOWNER} ] 
Bot admin UID: [${global.config.ADMINBOT} ]
