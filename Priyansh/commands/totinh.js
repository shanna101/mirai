...
async function makeImage({ one, two }) {
    ...
    totinh_img.resize(500, 500).composite(circleOne.resize(65, 65), 142, 86).composite(circleTwo.resize(65, 65), 293, 130);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "I like you❤ " + tag + '\n Do it for me UwU',
            mentions: [{
          tag: tag,
          id: mention
        }],
