...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  const Canvas = require('canvas');
    ...
    if(handleReply.step == 1){
      ...
      return api.sendMessage(`Bạn đã chọn char mang số thứ tự là ${event.body}, reply tin nhắn này để nhập tên của bạn`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 2,
      name: this.config.name,
      chartid: event.body,
      ...
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`Bạn đã chọn tên của mình là ${event.body}, reply tin nhắn này để nhập màu bạn muốn`, event.threadID, (err, info) => {
    return global.client.handleReply.push({
      step: 3,
      name: this.config.name,
      ...
      if(!color || color == "no" || color == "No"){
        const lengthchar = (await axios.get(`https://api.diencute1.repl.co/taoanhdep/search?type=id&id=${id}`)).data
      var color_ = lengthchar.colorBg
    } else {
      var color_ = color
    }
    const lengthcharr = (await axios.get(`https://api.diencute1.repl.co/taoanhdep/search?type=id&id=${id}`)).data
let avtAnime = (
      await axios.get(encodeURI(`${lengthcharr.imgAnime}`), { responseType: "arraybuffer" })).data;
